...
function AppContent() {
  ...
  const getNeonBorder = (tabName) => {
    if (activeTab === tabName) {
      switch (tabName) {
        case "wallet":
          return "border-b-2 border-fuchsia-500 shadow-fuchsia-500/50"
        case "creatures":
          return "border-b-2 border-cyan-400 shadow-cyan-400/50"
        case "battle":
          return "border-b-2 border-lime-400 shadow-lime-400/50"
        case "leaderboard":
          return "border-b-2 border-purple-500 shadow-purple-500/50"
        case "pokidesk":
          return "border-b-2 border-pink-500 shadow-pink-500/50"
        case "starter-picker":
          return "border-b-2 border-emerald-400 shadow-emerald-400/50"
        case "tcg":
          return "border-b-2 border-yellow-400 shadow-yellow-400/50"
        default:
          return "border-b-2 border-fuchsia-500"
      }
    }
    return ""
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 transition-all duration-300 ease-in-out">
      ...
      <header className="bg-gray-800/80 backdrop-blur-sm border-b border-gray-700 p-6 shadow-lg">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <h1
              className={`text-4xl font-bold ${colors.primary} ml-4 tracking-widest transition-all duration-300 hover:scale-105 hover:text-fuchsia-400`}
            >
              SuiMon
            </h1>
          </div>
            ...
            {isAuthenticated ? (
              <div className="flex items-center space-x-4">
                <ConnectButtonWrapper
                  className={`${bgColors.primary} hover:bg-fuchsia-600 px-6 py-3 rounded-lg font-bold shadow-lg shadow-fuchsia-500/30 hover:shadow-fuchsia-500/50 transition-all duration-300 transform hover:scale-105`}
                />
                <UserProfile />
              </div>
            ) : (
      ...
      {/* Navigation Tabs with neon highlight effect - Only shown when authenticated */}
      {isAuthenticated && (
      ...
      <main
        className={`container mx-auto p-6 transition-all duration-500 ${animate ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"}`}
      >
