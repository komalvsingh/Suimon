...
export default function Leaderboard() {
  ...
  const fetchCreatures = async () => {
    const all = await getOwnedCreatureNfts(wallet.account.address);
    const sorted = all.sort((a, b) => b.xp - a.xp); // Descending order
    setCreatures(sorted);
  };

  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6 text-center">🏆 Leaderboard (Your Creatures)</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {creatures.map((creature, index) => (
          ...
          >
            <span className="text-xl font-bold mb-2">#{index + 1}</span>
            <img
              src={creature.imageUrl}
              alt={creature.name}
              className="w-40 h-40 object-cover rounded"
            />
            <h2 className="text-lg font-bold mt-2">{creature.name}</h2>
            <p className="text-sm">Level: {creature.level}</p>
            <p className="text-sm">XP: {creature.xp}</p>
          </div>
        ))}
      </div>
    </div>
