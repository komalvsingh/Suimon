module suimon::starter_nft {
    ...
    // IPFS CIDs for metadata (to be replaced with actual CIDs after uploading to IPFS)
    const BULBASAUR_CID: vector<u8> = b"ipfs://bafkreicz5hnnlwvzztfsr257mc5vy2sitrwufr6bipmf7vwc56guxkn3sa";
    const CHARMANDER_CID: vector<u8> = b"ipfs://bafkreia5xsjwag54q5ck6vfmmtqg2fz2ohtap4y32ciucbd5ctvb6awwu4";
    const SQUIRTLE_CID: vector<u8> = b"ipfs://bafkreieo7na3patidayis4ndm665b3binl3aoexw63kvkwtcptjjntky3i";
    
    // Evolution CIDs (placeholder - replace with actual CIDs)
    const IVYSAUR_CID: vector<u8> = b"ipfs://bafkreifxd3qxqagt3rf5p2rl4ebwezla64ptlvnihv445rxjelqh2i3ne4";
    const VENUSAUR_CID: vector<u8> = b"ipfs://bafkreif534zlngrvv2dj44tpj6eo7fqwzxv46um5id4lq2du6flvn3ktga";
    const CHARMELEON_CID: vector<u8> = b"ipfs://bafkreihcxxmjqoldby6zybsupi2ba3z4ctpy5a2wmyjnipfam3ph2hmmfi";
    const CHARIZARD_CID: vector<u8> = b"ipfs://bafkreie4h2vmdxyypdaibpr4uucd554aivag4jbizmqv7r77oqxlfzkoha";
    const WARTORTLE_CID: vector<u8> = b"ipfs://bafkreihebgqi2emklknoldhebokigppmrao24ajwfmpp45autenhel43wi";
    const BLASTOISE_CID: vector<u8> = b"ipfs://bafkreigcyvwabmtoibojbtsdjxbgfu74upva7gifbqnbawfrd45zh2ubqu";
    ...
    fun mint_bulbasaur(recipient: address, ctx: &mut TxContext) {
        let nft = StarterNFT {
            ...
            token_uri: url::new_unsafe_from_bytes(BULBASAUR_CID),
            experience: 0,
            evolution_stage: 0,
        };
    ...
    fun mint_charmander(recipient: address, ctx: &mut TxContext) {
        let nft = StarterNFT {
            ...
            token_uri: url::new_unsafe_from_bytes(CHARMANDER_CID),
            experience: 0,
            evolution_stage: 0,
        };
    ...
    fun mint_squirtle(recipient: address, ctx: &mut TxContext) {
        let nft = StarterNFT {
            ...
            token_uri: url::new_unsafe_from_bytes(SQUIRTLE_CID),
            experience: 0,
            evolution_stage: 0,
        };
    ...
    fun evolve_pokemon(nft: &mut StarterNFT, new_stage: u8) {
        ...
        if (nft.pokemon_id == BULBASAUR_ID && new_stage == 1) {
            ...
            nft.token_uri = url::new_unsafe_from_bytes(IVYSAUR_CID);
            ...
            nft.token_uri = url::new_unsafe_from_bytes(VENUSAUR_CID);
            ...
            nft.token_uri = url::new_unsafe_from_bytes(CHARMELEON_CID);
            ...
            nft.token_uri = url::new_unsafe_from_bytes(CHARIZARD_CID);
            ...
            nft.token_uri = url::new_unsafe_from_bytes(WARTORTLE_CID);
            ...
            nft.token_uri = url::new_unsafe_from_bytes(BLASTOISE_CID);
        } else {
